...
from k_srme import glob2df, ID, DFT_NONAC_REF, DFT_NAC_REF
from k_srme.benchmark import get_metrics, process_benchmark_descriptors


model_name = "ORB-v1-MPTraj"

json_file = "k_srme.json.gz"
txt_path = "metrics.txt"
in_file = "conductivity_*-???.json.gz"

in_folder = f"2025-07-07-{model_name}-phononDB-LTC-FIRE_2SR_force0.0001_sym1e-05/"
...
if not glob(in_pattern):
    if os.path.exists(out_path):
        ...
        print(
            f"No MLP results found in {in_pattern} and {out_path} also does not exist."
        )
    print("a")
else:
